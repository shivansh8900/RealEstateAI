...
class PropertyChatbot:
    ...
    def parse_query(self, query: str) -> Dict:
        ...
        # City Extraction - Check ALL possible cities
    ...
    def filter_properties(self, filters: Dict) -> pd.DataFrame:
        ...
        if filters["city"] and not self.is_city_available(filters["city"]):
            # Return empty dataframe if city not available
            return pd.DataFrame()
        
        # Clean columns - handle missing values
        ...
        # Locality filter
        if filters["locality"] and 'address' in filtered.columns:
    ...
    def generate_summary(self, results: pd.DataFrame, filters: Dict) -> str:
        """Generate a natural language summary of results."""
        requested_city = filters.get("city", "").capitalize()
        bhk = filters.get("bhk", "")
        status = filters.get("status", "").replace("_", " ").title() if filters.get("status") else "available"
        budget = filters.get("budget")
        
        # Check if city was requested but not available
        ...
        # If no results even in available cities
        if results.empty:
            budget_str = ""
            ...
            return f"Unfortunately, no {bhk} {status.lower()} properties were found in {city_display} {budget_str}. Try adjusting your budget or removing some filters."
        ...
        if 'price_inr' in results.columns:
            avg_price = results['price_inr'].mean()
        else:
            avg_price = 0
        
        summary = f"Found {count} {bhk} {status.lower()} properties in **{city_display}** {budget_str}. "
        summary += f"The top match is **'{top_project}'** priced at {self.format_price(top_price)}. "
        if avg_price > 0:
            summary += f"Average price across all matches is {self.format_price(avg_price)}."
        
        return summary
    
    def process_query(self, query: str) -> Tuple[str, pd.DataFrame, Dict]:
        """Process user query and return summary, results, and filters."""
